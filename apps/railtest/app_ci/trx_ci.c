...
{
  if ((currentAppMode() != DIRECT) && (!inAppMode(NONE, argv[0])))
  {
    return;
  }
  ...
  if (enable && inRadioState(RAIL_RF_STATE_IDLE, argv[0]))
  {
    RAIL_RxStart(channel);
    responsePrint(argv[0], "Rx:Enabled,Idle:Disabled,Time:%u", RAIL_GetTime());
  }
  else if (!enable && inRadioState(RAIL_RF_STATE_RX, argv[0]))
  {
    RAIL_RfIdle();
    responsePrint(argv[0], "Rx:Disabled,Idle:Enabled,Time:%u", RAIL_GetTime());
  }
}
